...
static int st21nfca_loc_set_polaritymode(struct st21nfca_dev *st21nfca_dev, int mode)
{
	
	struct i2c_client *client = st21nfca_dev->client;
	static bool irqIsAttached = false;
...
#if (197632 > KERNEL_VERSION(2,6,36))
...
static void st21nfca_force_disable_irq(struct st21nfca_dev *st21nfca_dev)
{
	unsigned long flags;
	...
	disable_irq_wake(st21nfca_dev->client->irq);
...
#endif
{
	...
	case ST21NFCA_SET_POLARITY_FALLING:
	    ...
	    st21nfca_loc_set_polaritymode(st21nfca_dev, IRQF_TRIGGER_FALLING);
		break;
		
	case ST21NFCA_SET_POLARITY_RISING:
	    pr_info(" ### ST21NFCA_SET_POLARITY_RISING ###");
	    st21nfca_loc_set_polaritymode(st21nfca_dev, IRQF_TRIGGER_RISING);
		break;
		
	case ST21NFCA_SET_POLARITY_LOW:
	    pr_info(" ### ST21NFCA_SET_POLARITY_LOW ###");
	    st21nfca_loc_set_polaritymode(st21nfca_dev, IRQF_TRIGGER_LOW);
		break;
		
	case ST21NFCA_SET_POLARITY_HIGH:
	    pr_info(" ### ST21NFCA_SET_POLARITY_HIGH ###");
	    st21nfca_loc_set_polaritymode(st21nfca_dev, IRQF_TRIGGER_HIGH);
		break;
	...
	case ST21NFCA_DISABLE_IRQ:
	
		printk("%s(): ST21NFCA_DISABLE_IRQ  arg:%ld\n",
			__func__, arg);
		if(arg)
			st21nfca_force_disable_irq(st21nfca_dev);
		ret = 0;
		break;
		...
		const struct i2c_device_id *id)
{
  ...
  {	
   ret = gpio_request(platform_data->reset_gpio, "st21nfca_reset");
	if (ret){
...
err_exit:
	...
	if(platform_data->ena_gpio!=0)	gpio_free(platform_data->ena_gpio);
	return ret;
}
...
static int st21nfca_remove(struct i2c_client *client)
{
	...
	if(st21nfca_dev->ena_gpio!=0) gpio_free(st21nfca_dev->ena_gpio);
	kfree(st21nfca_dev);
